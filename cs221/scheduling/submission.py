...
def create_nqueens_csp(n = 8):
    ...
    vars = ['q%d' % i for i in range(n)]
    rows = [i for i in range(n)]
    for i in vars:
        csp.add_variable(i, rows)
    for i in range(len(vars)):
        for j in range(i+1, len(vars)):
            csp.add_binary_factor(vars[i], vars[j], lambda x, y: x != y and abs(x-y) != abs(i-j))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
        ...
        if not self.mcv:
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    currLen = cal_valid_domain_len(var)
                    if currLen < small:
                        picked = var
                        small = currLen
            return picked
    ...
    def arc_consistency_check(self, var):
        ...
        while len(queue) != 0:
            ...
            for other_var in self.csp.get_neighbor_vars(curr_var):
                new_domain = []
                for other_val in self.domains[other_var]:
                    for curr_val in self.domains[curr_var]:
                        if self.csp.binaryFactors[curr_var][other_var][curr_val][other_val] != 0:
                            new_domain.append(other_val)
                            break
                if len(new_domain) != len(self.domains[other_var]):
                    queue.append(other_var)
                self.domains[other_var] = new_domain
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    domains = [i for i in range(maxSum + 1)]
    csp.add_variable(result, domains)
    ...
    for i, X_i in enumerate(variables):
        ...
        if i == 0:
            ...
            csp.add_variable(A_i, [(j, k) for j in range(maxSum + 1)
                                   for k in range(maxSum + 1)])
            csp.add_binary_factor(('sum', name, i-1), A_i, lambda x, y: x[1] == y[0])
        csp.add_binary_factor(X_i, A_i, lambda x, y: x + y[0] == y[1])
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for request in self.profile.requests:
            for curr_quarter in request.quarters:
                for other_quarter in self.profile.quarters:
                    if curr_quarter == other_quarter or other_quarter in request.quarters: continue
                    csp.add_binary_factor((request, curr_quarter), (request, other_quarter), \
                                          lambda curr_cid, other_cid: \
                                          curr_cid is None and other_cid is None \
                                          or curr_cid is not None and other_cid is None)
        # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for quarter in self.profile.quarters:
            possible_vars = []
            for request in self.profile.requests:
                if len(request.quarters) > 0 and quarter not in request.quarters:
                    continue
                for cid in request.cids:
                    v = (cid, quarter)
                    minUnit, maxUnit = self.bulletin.courses[cid].minUnits, self.bulletin.courses[cid].maxUnits
                    unitVals = [i for i in range(minUnit, maxUnit + 1)] + [0]
                    csp.add_variable(v, unitVals)
                    # Do not forget to tie the new variable back to request assignment variable
                    # because each course can be requested at most once. If the current request assignment
                    # ends up not picking the course, it's abandoned in the quarter.
                    csp.add_binary_factor(v, (request, quarter), lambda units, request_cid: \
                        units > 0 if request_cid == cid else units == 0)
                    possible_vars.append(v)

            sumVar = get_sum_variable(csp, quarter, possible_vars, self.profile.maxUnits)
            csp.add_unary_factor(sumVar, lambda x: self.profile.minUnits <= x <= self.profile.maxUnits)
        # END_YOUR_CODE
