...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for r in range(self.belief.getNumRows()):
            for c in range(self.belief.getNumCols()):
                tileX, tileY = util.colToX(c), util.rowToY(r)
                actualDist = math.sqrt(((agentX - tileX) ** 2 + (agentY - tileY) ** 2))
                prob = util.pdf(actualDist, Const.SONAR_STD, observedDist)
                self.belief.setProb(r, c, prob * self.belief.getProb(r, c))
    ...
    def elapseTime(self):
        ...
        # you only need to iterate over transProb since anything not in here has 0 probability
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for tile in self.particles:
            actualDist = math.sqrt((agentX - util.colToX(tile[1])) ** 2 + (agentY - util.rowToY(tile[0])) ** 2)
            prob = util.pdf(actualDist, Const.SONAR_STD, observedDist)
            # note that self.particles may not be aligned with self.belief so
            # you should directly use particle distribution (instead of normalized belief)
            weights[tile] = self.particles[tile] * prob

        self.particles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            picked = util.weightedRandomChoice(weights)
            self.particles[picked] += 1
        # END_YOUR_CODE
