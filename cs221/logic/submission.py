...
def formula2c():
    ...
    def FemaleChild(x, y): return And(Female(x), Child(y, x))

    return Forall('$x', Forall('$y', Equiv(FemaleChild('$x', '$y'), Daughter('$y', '$x'))))
    # END_YOUR_CODE
...
def formula2d():
    ...
    return Forall('$x',
                  Forall('$z',
                         Equiv(And(Exists('$y', And(Parent('$x', '$y'), Parent('$y', '$z'))), Female('$z')),
                               Grandmother('$x', '$z'))))
    # END_YOUR_CODE
...
def ints():
    ...
    formulas.append(Forall('$x',
                           Forall('$y',
                                  Forall('$z', Implies(And(Larger('$x', '$y'), Larger('$y', '$z')),
                                                       Larger('$x', '$z'))))))
    # END_YOUR_CODE
...
def createRule1():
    ...
    return GrammarRule('$Clause', ['every', '$Noun', '$Verb', 'some', '$Noun'],
                       lambda args: Forall('$x', And(Atom(args[0].title(), '$x'),
